...
//    Handler shandler;
    ...
    public void run() {
        if (sIP.length() <= 0) {
            System.out.println("线程adminconnect, IP不能为空！\n");
            return;
        }
        try {
            //连接服务器
            ...
            System.out.println("线程adminconnect, 已经成功连接！");
        } catch (Exception e) {
            System.out.println("线程adminconnect, 连接异常:" + e.getMessage());
            return;
        }
        ...
        while (isConnecting) {
            try {
                if ((count = mBufferedReaderClient.read(buffer)) > 0) {
                    ...
                    System.out.println("线程adminconnect, 收到信息："+recvMessageClient);
                    receivemess(recvMessageClient);
                }
            } catch (Exception e) {
                System.out.println("线程adminconnect, 接收异常：" + e.getMessage());
            }
        }
    }
    ...
    public boolean sendstring(String ss) {
        if (isConnecting && mSocketClient != null) {
            if (ss.length() <= 0) {
                System.out.println("线程adminconnect, 发送内容不能为空！");
            } else {
                try {
                    System.out.println("线程adminconnect, 成功发送：" + ss);
                    mPrintWriterClient.print(ss);
                    mPrintWriterClient.flush();
                    return true;
                } catch (Exception e) {
                    System.out.println("线程adminconnect, 发送异常：" + e.getMessage());
                }
            }
        } else {
            System.out.println("线程adminconnect, 没有连接");
        }
        return false;
    }
